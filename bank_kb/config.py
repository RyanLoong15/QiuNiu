...
SILICONFLOW_API_URL = os.getenv(
    "SILICONFLOW_API_URL",
    "https://openapi.ai.jh/v1"
)

# LLM Fallback 链配置（模型级别 fallback）
# 主模型：DeepSeek-V3（性价比高，推理能力强）
# 备用模型：Qwen2.5-72B-Instruct（稳定可靠）
DEEPSEEK_MODEL = "DEEPSEEK-V4-284B-FLASH-V20260509"
QWEN_MODEL = "Qwen3-32B-20250627"
LLM_MODEL = DEEPSEEK_MODEL  # 内网模型
...
# 是否跳过 LLM 调用，直接返回检索结果（用于 LLM 异常时降级）
