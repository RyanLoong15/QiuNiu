...
<body>
    <div class="header">
        ...
        <div class="user-info">
            ...
            <a href="<%= request.getContextPath() %>/incidents.jsp" class="btn-logout">⚠️ 历史教训</a>
    ...
    <script>
        ...
        function saveConfig() {
            var value = document.getElementById('uploadPathInput').value.trim();
            if (!value) {
                showConfigStatus('请输入保存路径', 'error');
                return;
            }
            var btn = document.querySelector('#configModal .btn-primary');
            btn.disabled = true;
            btn.textContent = '保存中...';
            fetch(API_BASE + '/config/document_upload_path', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            .then(function(data) {
                if (data.success) {
                    showConfigStatus('✅ 保存成功！新导入的文档将保存到：' + value, 'success');
                } else {
                    showConfigStatus('❌ ' + (data.message || '保存失败'), 'error');
                }
            })
            .catch(function() {
