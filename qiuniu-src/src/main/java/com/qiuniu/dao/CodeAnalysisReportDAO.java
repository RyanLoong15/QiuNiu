...
public class CodeAnalysisReportDAO {
    ...
    public boolean finish(Long id, String report, String incidentCheckResult, int totalIssues, long costMs) {
        String sql = "UPDATE code_analysis_reports " +
                "SET status='DONE', report=?, incident_check_result=?, total_issues=?, finished_at=NOW(), cost_ms=? WHERE id=?";
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    /** 更新指定报告的历史事故风险检核结果 */
    public boolean updateIncidentCheckResult(Long id, String incidentCheckResult) {
        String sql = "UPDATE code_analysis_reports SET incident_check_result=? WHERE id?";
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, incidentCheckResult);
            ps.setLong(2, id);
            return ps.executeUpdate() > 0;
        } catch (Exception e) {
            e.printStackTrace();
