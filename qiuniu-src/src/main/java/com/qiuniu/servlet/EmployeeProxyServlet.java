...
/**
 * 代理 /api/employee/* 请求到 Flask (http://localhost:5001/api/employee/*)
 */
@WebServlet("/api/employee/*")
public class EmployeeProxyServlet extends HttpServlet {

    private static final String FLASK_BASE = "http://localhost:5001";
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            URL url = new URL(FLASK_BASE + flaskPath);

            // 处理查询参数
            String queryString = req.getQueryString();
            if (queryString != null && !queryString.isEmpty()) {
                url = new URL(FLASK_BASE + flaskPath + "?" + queryString);
            }
