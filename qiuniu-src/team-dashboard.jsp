...
<body>
    ...
    <div class="modal" id="aiModal">
        <div class="modal-content">
            <div class="modal-header">
                <h2 id="aiModalTitle">AI 风险分析</h2>
                <button class="modal-close" onclick="closeAiModal()">x</button>
            </div>
            <div class="modal-body">
                <div id="aiModalContent" style="white-space:pre-wrap;line-height:1.6;font-size:14px;">加载中...</div>
                <div id="aiIncidentSection" style="margin-top:16px;display:none;"></div>
            </div>
        </div>
    </div>
    ...
    <script>
                ...
                for (var i = 0; i < items.length; i++) {
                    ...
                    html += '<div class="diff-accordion-item">' +
                        ...
                        '<div class="diff-accordion-body">' +
                            ...
                            '<div style="margin-bottom:10px;"><button class="btn-ai-analyze" onclick="triggerAiAnalysis(\'' + escHtml(item.projectName || '') + '\', \'' + escHtml(item.baseBranch || '') + '\', \'' + escHtml(item.compareBranch || '') + '\', ' + i + ')">🤖 AI风险分析</button></div>' +
                            '<pre id="diffContent_' + i + '" style="white-space:pre-wrap;font-size:12px;color:#888;max-height:300px;overflow:auto;">' + escHtml(diff) + '</pre>' +
                        '</div>' +
                    '</div>';
                }
        ...
        function triggerAiAnalysis(projectName, baseBranch, compareBranch, diffIdx) {
            var diff = '';
            if (typeof diffIdx === 'number') {
                var preEl = document.getElementById('diffContent_' + diffIdx);
                if (preEl) diff = preEl.textContent || '';
            }
            ...
            .then(function(data) {
                if (data.success && data.data) {
                    document.getElementById('aiModalTitle').textContent = 'AI 风险分析: ' + (data.data.projectName || projectName);
                    document.getElementById('aiModalContent').textContent = data.data.analysis || '无分析结果';
                    var incidentSection = document.getElementById('aiIncidentSection');
                    if (data.data.incidentCheckResult && data.data.incidentCheckResult.trim()) {
                        incidentSection.style.display = 'block';
                        incidentSection.innerHTML = '<div style="background:#fff3cd;border:1px solid #ffc107;border-radius:8px;padding:12px 14px;white-space:pre-wrap;line-height:1.6;font-size:13px;">' + data.data.incidentCheckResult.replace(/\n/g, '<br>') + '</div>';
                    } else {
